/**
 * Main angularjs file. Defines the "library" module and the mapping from
 * routes to views.
 */

/**
 * An annoying browser-based beep used as an audio cue when a wrong key is
 * pressed.  See http://stackoverflow.com/a/23395136
 */
function beep() {
    var snd = new Audio("data:audio/wav;base64,");
    snd.play();
}

angular.module("library", ["ngRoute", 'ui.bootstrap'])
.config(function ($routeProvider) {
  $routeProvider
    .when("/dashboard", { templateUrl: "views/dashboard.html" })
    .when("/borrower", { templateUrl: "views/borrower.html" })
    .when("/items", { templateUrl: "views/items.html" })
    .when("/returns", { templateUrl: "views/returns.html" })
    .when("/item/new", { templateUrl: "views/item_new.html" });
})

/**
 * Utility service providing helper functions.
 */
.factory('util', function () {
  /**
   * Returns object as string, turning null and undefined into an empty string.
   */
  function str(o) { return o ? String(o) : ''; }

  function add(a, b) { return a + b; }

  function sum(values) { return values.reduce(add, 0); }

  /**
   * Returns the getter function for a field.
   */
  function getter(field) {
    return function (obj) { return obj[field]; };
  }

  /**
   * Returns the setter function for a field.
   */
  function setter(field) {
    return function (obj, value) { obj[field] = value; };
  }

  /**
   * Returns the string combining the given fields of an object, separator
   * by the separator string (which defaults to ", ").
   */
  function joinFields(obj, fields, separator) {
    return fields.map(
      function (field) { return str(obj[field]); }).join(separator || ", ");
  }

  return {
    str: str,
    add: add,
    sum: sum,
    getter: getter,
    setter: setter,
    joinFields: joinFields,
    beep: beep
  };
});
